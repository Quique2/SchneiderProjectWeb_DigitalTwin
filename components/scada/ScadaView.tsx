...
import { ScadaLang, readStoredLang, writeStoredLang, tsc, ScadaLangCtx, useScadaT, LANG_LABELS } from './scadaI18n';
...
function buildSections(lang: ScadaLang): { id: Section; label: string }[] {
  return [
    { id: 'overview', label: tsc(lang, 'tabOverview') },
    { id: 'scada40',  label: tsc(lang, 'tabScada40') },
    { id: 'cafis',    label: tsc(lang, 'tabCafis') },
    { id: 'io',       label: tsc(lang, 'tabIo') },
    { id: 'alarms',   label: tsc(lang, 'tabAlarms') },
    { id: 'cobot',    label: tsc(lang, 'tabCobot') },
    { id: 'trends',   label: tsc(lang, 'tabTrends') },
  ];
}
...
export default function ScadaView() {
  ...
  const [lang, setLangState] = useState<ScadaLang>(readStoredLang);
  const changeLang = (l: ScadaLang) => { setLangState(l); writeStoredLang(l); };
  const t = (key: string) => tsc(lang, key);
  ...
  return (
    <ScadaLangCtx.Provider value={lang}>
    <div style={{ ...cssVars(theme), height: '100%', display: 'flex', flexDirection: 'column', background: C.bg, color: C.text, fontFamily: SANS, overflow: 'hidden' }}>
      ...
      <div style={{ flexShrink: 0, display: 'flex', alignItems: 'center', gap: 12, padding: '11px 18px', borderBottom: `1px solid ${C.border}`, background: C.panel2, flexWrap: 'wrap' }}>
        <div style={{ display: 'flex', flexDirection: 'column' }}>
          ...
          <div style={{ fontSize: 10.5, color: C.muted, letterSpacing: 0.4 }}>{t('subtitle')} · <span style={{ color: SRC.REAL, fontWeight: 700 }}>REAL MODE</span></div>
        </div>
        <PlantPill state={snapshot.overview.plantState} />
        <div style={{ flex: 1 }} />
        <LinkChip label="Cobot" link={snapshot.cobotLink} />
        <LinkChip label={t('sistemaIo')} link={snapshot.plcLink} />
        {/* Language selector */}
        <div style={{ display: 'flex', gap: 2 }}>
          {(Object.keys(LANG_LABELS) as ScadaLang[]).map((l) => (
            <button key={l} onClick={() => changeLang(l)}
              style={{ background: lang === l ? SRC.CONFIGURED + '22' : 'transparent', color: lang === l ? SRC.CONFIGURED : C.muted, border: `1px solid ${lang === l ? SRC.CONFIGURED + '66' : C.border}`, borderRadius: 2, padding: '3px 6px', fontSize: 9.5, fontWeight: 700, cursor: 'pointer', fontFamily: MONO, letterSpacing: 0.5 }}>
              {LANG_LABELS[l]}
            </button>
          ))}
        </div>
        <ScadaThemeToggle />
        <DemoToggle on={demo} onToggle={() => setDemo((v) => !v)} lang={lang} />
        <button onClick={runAi} style={primaryBtn} title={t('analyzeAi')}>{t('analyzeAi')}</button>
      </div>
      ...
      <div style={{ flexShrink: 0, display: 'flex', alignItems: 'center', gap: 2, padding: '0 12px', borderBottom: `1px solid ${C.border}`, background: C.panel2 }}>
        ...
        <div style={{ flex: 1 }} />
        <div style={{ fontSize: 11, color: C.muted, marginRight: 10 }}>{t('activeAlarmsCount').replace('{n}', String(activeAlarms.length))}</div>
        <button onClick={() => setShowTags((v) => !v)} style={ghostBtn(showTags)} title="Advanced tags">{showTags ? t('hideTags') : t('advancedTags')}</button>
      </div>
...
function Overview({ s, ai, aiHealth }: { s: ScadaSnapshot; ai: AiDiagnosis | null; aiHealth: AiBackendHealth }) {
  ...
  const metrics: MetricDef[] = [
    { label: t('metric1'), value: o.plantState, color: plantCol(o.plantState) },
    { label: t('metric2'), value: o.stage ?? 'N/D', color: o.stage ? C.text : C.dim, text: true,
      sub: o.stage ? (o.stageEstimated ? t('estimatedByIo') : undefined) : t('noSignal') },
    { label: t('metric3'), value: o.cycleStep !== null && o.cycleTotal !== null ? `${o.cycleStep}/${o.cycleTotal}` : 'N/D',
      color: o.cycleStep !== null ? C.text : C.dim, progress: (o.cycleStep !== null && o.cycleTotal) ? o.cycleStep / o.cycleTotal : null,
      sub: o.cycleStep === null ? t('noSignal') : (o.stageEstimated ? t('estimatedByIo') : undefined) },
    { label: t('metric4'), value: String(o.activeAlarms), color: o.activeAlarms ? (o.worstSeverity ? SEV[o.worstSeverity] : SEV.ALARM) : SRC.REAL,
      sub: o.activeAlarms ? `${t('worst')} ${o.worstSeverity}` : t('noAlarms') },
    { label: t('metric5'), value: o.cycleTimeS !== null ? `${o.cycleTimeS}s` : 'N/D', color: o.cycleTimeS !== null ? C.text : C.dim,
      sub: o.cycleTimeS === null ? t('noSignal') : (o.cycleRunning ? t('inProgress') : t('lastCycle')) },
    { label: t('metric6'), value: o.avgCycleTimeS !== null ? `${o.avgCycleTimeS}s` : '—', color: o.avgCycleTimeS !== null ? C.text : C.dim,
      sub: o.avgCycleTimeS === null ? t('noHistory') : t('ncycles').replace('{n}', String(s.history.cycleTimes.length)) },
    { label: t('metric7'), value: conv.signalAvailable ? `${conv.onTimeS}s` : 'NO CONEC.', text: !conv.signalAvailable,
      color: !conv.signalAvailable ? C.dim : conv.warning === 'ALARM' ? SEV.ALARM : conv.warning === 'WARNING' ? SEV.WARNING : SRC.REAL,
      sub: conv.signalAvailable ? undefined : t('motorSignal') },
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 16, width: '100%' }}>
      ...
      {o.stageEstimated && s.mode === 'REAL' && (
        <Banner color={C.dim} text={t('stageEstimatedBanner')} />
      )}
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 14 }}>
        <Panel title={t('panelComms')}>
          <KV k={t('cobotWs')} v={<SourceBadge src={s.cobotLink.state} />} sub={s.cobotLink.ageS !== null ? t('lastFrame').replace('{n}', String(s.cobotLink.ageS)) : t('noFrame')} />
          <KV k={t('sistemaIo')} v={<SourceBadge src={s.plcLink.state} />} sub={s.plcLink.ageS !== null ? t('lastFrame').replace('{n}', String(s.plcLink.ageS)) : t('waitingTelemetry')} />
          <KV k={t('aiBackend')} v={<AiHealthBadge h={aiHealth} />} />
          <KV k={t('modeLabel')} v={<SourceBadge src={s.mode === 'DEMO' ? 'DEMO' : 'REAL'} />} />
        </Panel>
        <Panel title={t('panelActiveAlarms').replace('{n}', String(o.activeAlarms))}>
          {s.alarms.filter((a) => a.active).slice(0, 6).map((a) => <AlarmRow key={a.id} a={a} />)}
          {o.activeAlarms === 0 && <Empty>{t('noActiveAlarms')}</Empty>}
        </Panel>
        <Panel title={t('panelLastAiDiag')}>
          {ai ? (
            <div>
              <div style={{ display: 'flex', gap: 6, marginBottom: 6, flexWrap: 'wrap' }}>
                <SeverityBadge sev={ai.severity} />
                <Chip color={ai.source === 'AI' ? SRC.REAL : SRC.DEMO} text={ai.source === 'AI' ? 'OpenAI' : 'Mock local'} />
                <Chip color={ai.canKeepOperating ? SRC.REAL : SEV.CRITICAL} text={ai.canKeepOperating ? t('canOperate') : t('stopCmd')} />
              </div>
              <div style={{ fontSize: 12, color: C.text }}>{ai.probableFault}</div>
            </div>
          ) : <Empty>{t('noAiDiag')}</Empty>}
        </Panel>
      </div>

      {!o.plcConnected && s.mode === 'REAL' && (
        <Banner color={C.dim} text={t('ioNotConnectedBanner')} />
      )}
    </div>
  );
}
...
function ProductionPanel({ p }: { p: ProductionSnap }) {
  ...
  return (
    <Panel title={t('panelProduction')}>
      <div style={{ display: 'flex', gap: 10, alignItems: 'stretch', flexWrap: 'wrap' }}>
        {stat(t('prodTotal'), na(p.total), col)}
        {stat(t('prodAccepted'), na(p.accepted), p.accepted !== null ? SRC.REAL : C.dim)}
        {stat(t('prodRejected'), na(p.rejected), p.rejected ? SEV.ALARM : p.rejected === 0 ? SRC.REAL : C.dim)}
        {stat(t('prodYield'), p.yieldPct !== null ? `${p.yieldPct}%` : 'N/A', p.yieldPct === null ? C.dim : p.yieldPct >= 95 ? SRC.REAL : p.yieldPct >= 85 ? SEV.WARNING : SEV.ALARM)}
        <div style={{ background: C.panel2, padding: '12px 14px', border: `1px solid ${C.borderSoft}`, borderRadius: 2, flex: 1, minWidth: 120, display: 'flex', flexDirection: 'column', justifyContent: 'space-between' }}>
          <div style={{ ...labelStyle, fontSize: 9 }}>{t('prodCamera')}</div>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginTop: 10 }}>
        ...
        <span style={{ fontSize: 9.5, color: C.dim }}>
          {p.available ? t('prodAvailable') : t('prodWaiting')}
        </span>
      </div>
    </Panel>
  );
...
function AlarmsMaint({ s, acknowledge, acknowledgeAll, resolve, addComment, adminDraft, setAdminDraft }: {
  ...
  const fireEmail = async (txt: string, code: string | null) => {
    ...
    setEmailStatus(r.via === 'auto' ? t('emailSent') : r.via === 'gmail' ? t('emailGmail') : null);
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 16, width: '100%' }}>
      <div style={{ display: 'flex', gap: 8, alignItems: 'center', flexWrap: 'wrap' }}>
        ...
        <div style={{ flex: 1 }} />
        <button onClick={acknowledgeAll} style={ghostBtn(false)}>{t('ackAll')}</button>
      </div>

      <Panel title={t('panelAlarms').replace('{n}', String(s.alarms.length))}>
        <div style={{ display: 'grid', gridTemplateColumns: '84px 1fr 110px 110px 200px', gap: 6, ...labelStyle, padding: '0 0 8px', borderBottom: `1px solid ${C.border}` }}>
          <div>{t('colSeverity')}</div><div>{t('colCodeMsg')}</div><div>{t('colStatus')}</div><div>{t('colStationDur')}</div><div>{t('colActions')}</div>
        </div>
        {s.alarms.length === 0 && <Empty>{t('noAlarmsRegistered')}</Empty>}
        {s.alarms.map((a) => (
          <div key={a.id} style={{ borderBottom: `1px solid ${C.borderSoft}`, padding: '7px 0', opacity: a.status === 'RESOLVED' ? 0.5 : 1 }}>
            <div style={{ display: 'grid', gridTemplateColumns: '84px 1fr 110px 110px 200px', gap: 6, alignItems: 'center' }}>
              ...
              <div style={{ fontSize: 10, color: C.muted }}>{a.station}<div style={{ fontFamily: MONO }}>{a.active ? `${Math.round(a.durationS)}s` : t('inactive')}</div></div>
              <div style={{ display: 'flex', gap: 4, flexWrap: 'wrap' }}>
                <button onClick={() => acknowledge(a.id)} disabled={a.status !== 'ACTIVE'} style={miniBtn(a.status !== 'ACTIVE')}>{t('ackBtn')}</button>
                <button onClick={() => doResolve(a)} disabled={a.active || a.status === 'RESOLVED'}
                  style={miniBtn(a.active || a.status === 'RESOLVED', SRC.REAL)}>{t('resolvedBtn')}</button>
                <button onClick={() => { setOpenComment(openComment === a.id ? null : a.id); setCommentDraft(a.operatorComment ?? ''); }} style={miniBtn(false)}>{t('commentBtn')}</button>
              </div>
            </div>
            ...
            {openComment === a.id && (
              <div style={{ marginTop: 6, display: 'flex', gap: 6 }}>
                <textarea value={commentDraft} onChange={(e) => setCommentDraft(e.target.value)} rows={2}
                  ...
                  style={inputStyle} />
                <button onClick={() => sendComment(a.code)} disabled={!commentDraft.trim()} style={{ ...primaryBtn, alignSelf: 'stretch' }}>{t('sendToAdmin')}</button>
              </div>
            )}
          </div>
        ))}
      ...
      <Panel title={t('panelOpComment')}>
        <div style={{ display: 'flex', gap: 8, alignItems: 'flex-start' }}>
          ...
          <button onClick={() => { const txt = adminDraft.trim(); if (!txt) return; addComment(txt, null); void fireEmail(txt, null); setAdminDraft(''); }} disabled={!adminDraft.trim()}
            style={{ ...primaryBtn, alignSelf: 'stretch' }}>{t('sendToAdmin')}</button>
        </div>
        ...
        <div style={{ fontSize: 9.5, color: C.dim, marginTop: 6 }}>
          {t('emailFooter')} <b>{ADMIN_EMAIL}</b> {t('emailFooter2')}
        </div>
      </Panel>

      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2,1fr)', gap: 14 }}>
        <Panel title={t('panelMaintHistory').replace('{n}', String(s.maintenanceLog.length))}>
          {s.maintenanceLog.length === 0 && <Empty>{t('noMaintLog')}</Empty>}
          {s.maintenanceLog.slice(0, 12).map((e) => <LogRow key={e.id} e={e} now={s.ts} />)}
        </Panel>
        <Panel title={t('panelMaintData')}>
          {m.items.length === 0 && <Empty>{t('noMaintData')}</Empty>}
          {m.items.map((it) => <MaintRow key={it.label} it={it} />)}
          {pending.length > 0 && (
            <div style={{ marginTop: 10 }}>
              <div style={{ ...labelStyle, marginBottom: 6 }}>{t('pendingIntegrations').replace('{n}', String(pending.length))}</div>
              {m.checklist.map((c) => (
...
function StatusBadge({ a }: { a: Alarm }) {
  ...
  const label = a.status === 'ACTIVE' ? (a.active ? t('alarmActive') : t('alarmPendResolve'))
    : a.status === 'ACKNOWLEDGED' ? (a.active ? t('alarmAcked') : t('alarmAckedInactive'))
    : t('alarmResolved');
  ...
  return (
    <div style={{ padding: '6px 0', borderBottom: `1px solid ${C.borderSoft}` }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
        ...
        <span style={{ fontSize: 9.5, color: C.dim, fontFamily: MONO }}>{e.by} · {t('logAgo').replace('{n}', String(ago))}{e.alarmCode ? ` · ${e.alarmCode}` : ''}</span>
      </div>
...
function CobotRecovery({ url, online, demo, faulted }: { url: string; online: boolean; demo: boolean; faulted: boolean }) {
  ...
  return (
    <Panel title={t('panelRecovery')}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 10, flexWrap: 'wrap' }}>
        <div style={{ flex: 1, minWidth: 220 }}>
          <div style={{ fontSize: 11.5, color: C.text }}>{t('recoveryTitle')}</div>
          <div style={{ fontSize: 9.5, color: C.dim, marginTop: 2 }}>
            {t('recoveryDesc')}
          </div>
        </div>
        {faulted && !demo && <Chip color={SEV.ALARM} text={t('activeFault')} />}
        {!armed ? (
          <button onClick={() => { setResult(null); setArmed(true); }} disabled={blocked || busy}
            title={demo ? t('demoBlocked') : !online ? t('cobotNotConnected') : t('recoveryTitle')}
            style={miniBtn(blocked || busy, SEV.WARNING)}>
            {busy ? t('sending') : t('resetBtn')}
          </button>
        ) : (
          <span style={{ display: 'flex', gap: 6, alignItems: 'center' }}>
            <span style={{ fontSize: 10.5, color: SEV.WARNING, fontFamily: MONO }}>{t('confirmPrompt')}</span>
            <button onClick={run} style={miniBtn(false, SEV.ALARM)}>{t('yesReset')}</button>
            <button onClick={() => setArmed(false)} style={miniBtn(false)}>{t('cancelBtn')}</button>
          </span>
        )}
      </div>
      {demo && <div style={{ fontSize: 9.5, color: SRC.DEMO, marginTop: 8, fontFamily: MONO }}>{t('demoBlocked')}</div>}
      {result && (
...
function CobotHealth({ c, gatewayUrl, online }: { c: CobotHealthSnap; gatewayUrl: string; online: boolean }) {
  const t = useScadaT();
  if (!c.available) return <Banner color={C.dim} text={t('cobotNotConnected')} />;
  const tempOut = (v: number) => v > 70 ? SEV.CRITICAL : v > 60 ? SEV.ALARM : v > 50 ? SEV.WARNING : null;
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 16, width: '100%' }}>
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', gap: 1, background: C.border, border: `1px solid ${C.border}` }}>
        <Kpi label={t('kpiMaxTemp')} value={c.maxJointTempC !== null ? `${c.maxJointTempC}°C` : 'N/D'} color={c.maxJointTempC !== null && tempOut(c.maxJointTempC) ? tempOut(c.maxJointTempC)! : SRC.REAL} />
        <Kpi label={t('kpiController')} value={c.controllerTempC !== null ? `${c.controllerTempC}°C` : 'N/D'} color={SRC.CONFIGURED} />
        <Kpi label={t('kpiSpeed')} value={c.speedMagnificationPct !== null ? `${c.speedMagnificationPct}%` : 'N/D'} color={C.text} />
        <Kpi label={t('kpiAvgCurrent')} value={c.avgCurrentA !== null ? `${c.avgCurrentA} A` : 'N/D'} color={C.text} />
      </div>

      <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap' }}>
        <Chip color={c.flags.enabled ? SRC.REAL : C.dim} text={`enabled: ${c.flags.enabled ? 'yes' : 'no'}`} />
        <Chip color={c.flags.powerOn ? SRC.REAL : C.dim} text={`power: ${c.flags.powerOn ? 'on' : 'off'}`} />
        <Chip color={c.flags.moving ? SRC.CONFIGURED : C.dim} text={`moving: ${c.flags.moving ? 'yes' : 'no'}`} />
        <Chip color={c.flags.estop ? SEV.CRITICAL : C.dim} text={`E-stop: ${c.flags.estop ? 'ACTIVE' : 'no'}`} />
        <Chip color={c.flags.protectiveStop ? SEV.ALARM : C.dim} text={`prot.stop: ${c.flags.protectiveStop ? 'yes' : 'no'}`} />
        <Chip color={c.flags.collision ? SEV.ALARM : C.dim} text={`collision: ${c.flags.collision ? 'yes' : 'no'}`} />
        <Chip color={c.flags.softLimit ? SEV.WARNING : C.dim} text={`soft limit: ${c.flags.softLimit ? 'yes' : 'no'}`} />
      ...
      <Panel title={t('panelJoints')}>
        <div style={{ display: 'grid', gridTemplateColumns: '60px 1fr 1fr 1fr', gap: 6, ...labelStyle, padding: '0 0 8px', borderBottom: `1px solid ${C.border}` }}>
          <div>{t('colJoint')}</div><div>{t('colAngle')}</div><div>{t('colTemp')}</div><div>{t('colCurrent')}</div>
        </div>
        {c.joints.map((j) => {
          const oc = tempOut(j.tempC);
          return (
...
function Trends({ s }: { s: ScadaSnapshot }) {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 16, width: '100%' }}>
      <Panel title={t('trendConveyorLive')}>
        {!s.conveyor.signalAvailable ? <Empty>{t('conveyorNoSignal')}</Empty>
          : h.conveyorOnTime.length >= 2
            ...
            : <div style={{ fontSize: 26, fontFamily: MONO, color: SRC.CONFIGURED }}>{s.conveyor.onTimeS}s <span style={{ fontSize: 11, color: C.dim }}>{t('conveyorAccumulating')}</span></div>}
        <div style={{ fontSize: 9.5, color: C.dim, marginTop: 6 }}>{t('trendConveyorDesc')}</div>
      </Panel>

      <Panel title={t('trendConveyorHistory')}>
        {!s.conveyor.signalAvailable ? <Empty>{t('conveyorNoSignal')}</Empty>
          : h.conveyorOnDurations.length === 0 ? <Empty>{t('noRealHistory')}</Empty>
            : <BarChart values={h.conveyorOnDurations} unit="s" warnAt={60} alarmAt={120} labels={h.conveyorOnDurations.map((_, i) => `#${i + 1}`)} />}
        <div style={{ fontSize: 9.5, color: C.dim, marginTop: 6 }}>{t('trendConveyorHistDesc')}</div>
      </Panel>

      <Panel title={t('trendCycleTimes')}>
        {h.cycleTimes.length === 0
          ? <Empty>{t('cycleHistoryEmpty')} {s.mode === 'REAL' ? t('cycleHistoryEmptySuffix') : ''}</Empty>
          : <BarChart values={h.cycleTimes} unit="s" labels={h.cycleTimes.map((_, i) => `#${i + 1}`)} />}
        <div style={{ fontSize: 9.5, color: C.dim, marginTop: 6 }}>{t('cycleHistoryDesc')} {s.overview.avgCycleTimeS !== null ? `${t('cycleAvgSuffix')} ${s.overview.avgCycleTimeS}s.` : ''}</div>
      </Panel>

      <Panel title={t('trendJointTemps')}>
        {jointSeries ? <LineChart series={jointSeries} unit="°C" refs={[{ v: 50, color: SEV.WARNING }, { v: 60, color: SEV.ALARM }, { v: 70, color: SEV.CRITICAL }]} />
          : jointBars.length ? <BarChart values={jointBars.map((b) => b.value)} labels={jointBars.map((b) => b.label)} unit="°C" warnAt={50} alarmAt={60} />
            : <Empty>{t('cobotNoJointTemps')}</Empty>}
        ...
        <div style={{ fontSize: 9.5, color: C.dim, marginTop: 6 }}>{t('jointTempThresholds')}</div>
      </Panel>

      <Panel title={t('trendControllerTemp')}>
        {ctrlSeries ? <LineChart series={ctrlSeries} unit="°C" refs={[{ v: 50, color: SEV.WARNING }, { v: 60, color: SEV.ALARM }]} />
          : s.cobot.controllerTempC !== null ? <div style={{ fontSize: 26, fontFamily: MONO, color: SRC.CONFIGURED }}>{s.cobot.controllerTempC}°C <span style={{ fontSize: 11, color: C.dim }}>{t('controllerAccumulating')}</span></div>
            : <Empty>{t('cobotNoCtrlTemp')}</Empty>}
        <div style={{ fontSize: 9.5, color: C.dim, marginTop: 6 }}>{`${t('cycleHistoryDesc')} ${h.controllerTemps.length}pts. 50°C / 60°C.`}</div>
      </Panel>
    </div>
  );
}
...
function TagTable({ tags }: { tags: Tag[] }) {
  ...
  const f = tags.filter((tag) => tag.name.toLowerCase().includes(q.toLowerCase()));
  return (
    <Panel title={t('advancedTagsTitle').replace('{n}', String(f.length)).replace('{m}', String(tags.length))}>
      <div style={{ fontSize: 11, color: C.dim, marginBottom: 10 }}>{t('tagsDesc')}</div>
      <input value={q} onChange={(e) => setQ(e.target.value)} placeholder={t('filterTagsPlaceholder')}
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 130px 70px 100px', gap: 6, ...labelStyle, padding: '0 0 8px', borderBottom: `1px solid ${C.border}` }}>
        <div>{t('colTag')}</div><div>{t('colValue')}</div><div>{t('colUnit')}</div><div>{t('colSource')}</div>
      </div>
      <div style={{ maxHeight: '64vh', overflowY: 'auto' }}>
        {f.length === 0 && <Empty>{t('noTagsData')}</Empty>}
        {f.map((t) => (
  ...
  return (
    <div style={{ width: 380, flexShrink: 0, borderLeft: `1px solid ${C.border}`, background: C.panel2, overflowY: 'auto', display: 'flex', flexDirection: 'column' }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '12px 14px', borderBottom: `1px solid ${C.border}` }}>
        <div style={{ fontSize: 12, fontWeight: 700, letterSpacing: 1, textTransform: 'uppercase', flex: 1 }}>{t('maintenanceCopilot')}</div>
        <AiHealthBadge h={health} />
        <button onClick={onClose} style={ghostBtn(false)} aria-label={t('closePanelAi')}>✕</button>
      </div>
      <div style={{ padding: 14, display: 'flex', flexDirection: 'column', gap: 12 }}>
        {mode === 'DEMO' && <Banner color={SRC.DEMO} text={t('demoSnapshot')} />}
        {!loading && !ai && (
          <>
            <Empty>{t('noAiDiagDrawer')}</Empty>
            <button onClick={onRun} style={{ ...primaryBtn, width: '100%', padding: '12px 0', fontSize: 13 }}>{t('analyzeCurrentState')}</button>
            <div style={{ fontSize: 9.5, color: C.dim }}>{t('aiDesc')}</div>
          </>
        )}
        {loading && <Empty>{t('sendingToAi')}</Empty>}
        {!loading && ai && (
          <>
            <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap' }}>
              <SeverityBadge sev={ai.severity} />
            ...
            <AiCard label={t('aiCardFault')} text={ai.probableFault} />
            {ai.suspectComponent && <AiCard label={t('aiCardComponent')} text={ai.suspectComponent} />}
            <AiCard label={t('aiCardAction')} text={ai.recommendedAction} />
            {ai.evidence.length > 0 && <AiCard label={t('aiCardEvidence')} text={ai.evidence.join('\n')} mono />}
            {ai.checklist.length > 0 && (
              <div style={cardBox}>
                <div style={labelStyle}>{t('aiCardChecklist')}</div>
            ...
            {ai.maintenanceMessage && <AiCard label={t('aiCardMessage')} text={ai.maintenanceMessage} mono />}
            <button onClick={() => onUseAsMessage(ai.maintenanceMessage || ai.probableFault)} style={{ ...primaryBtn, width: '100%' }}>
              {t('useAsMessage')}
            </button>
            <button onClick={onRun} style={ghostBtn(false)}>{t('reanalyze')}</button>
          </>
        )}
      </div>
    </div>
...
function AiHealthBadge({ h }: { h: AiBackendHealth }) {
  const t = useScadaT();
  const map: Record<AiBackendHealth['state'], { c: string; label: string }> = {
    CONFIGURED:     { c: SRC.REAL,  label: 'CONFIGURED' },
    NOT_CONFIGURED: { c: SRC.STALE, label: t('sinKey') },
    NOT_CONNECTED:  { c: C.dim,     label: t('noConnected') },
    UNKNOWN:        { c: C.dim,     label: '…' },
  };
  const x = map[h.state];
...
function DemoToggle({ on, onToggle, lang }: { on: boolean; onToggle: () => void; lang: ScadaLang }) {
  return (
    <button onClick={onToggle} title={tsc(lang, 'demoTooltip')}
      ...
      <Dot on={on} color={SRC.DEMO} /> {on ? tsc(lang, 'demoOn') : tsc(lang, 'demoOff')}
    </button>
  );
}
